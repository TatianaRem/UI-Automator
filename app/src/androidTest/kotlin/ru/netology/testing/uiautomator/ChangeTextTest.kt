...
//    }
    ...
    fun testEmptyText() {
        ...
        val originalText = "Hello UiAutomator!"
        assertEquals(originalText, result)

    }

    @Test
    fun testStartNewActivityAndVerifyText() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        val inputText = "Любой текст"

        device.findObject(By.res(packageName, "userInput")).text = inputText

        device.findObject(By.res(packageName, "buttonChange")).click()

        val secondActivityTextView = device.wait(
            Until.findObject(By.res(packageName, "textToBeChanged")),
            TIMEOUT
        )
        val resultText = secondActivityTextView.text

        val expectedText = "Любой текст"
